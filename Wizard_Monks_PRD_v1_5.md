...
- *Social and relationship goals*: Generated from the full range of interpersonal emotion tokens, mediated by Sociability, Fairness, Patience, and Forgivingness. The generative conditions are richer than cooperative goals alone:
  ...
  Two concrete social goal types are implemented but not yet wired into the GoalGenerator: `FindApprenticeGoal` (a mage old enough and socially motivated enough to want a student) and `FoundCovenantGoal` (a Visitor with sufficient magical development and independence drive seeking to establish their own covenant). Both belong in `TryGenerateSocialIntentions()`. The generative conditions for each are specified in §2.9.6.
...
**Spell Use Tags.** Each spell base carries a set of `SpellUseTag` values — a vocabulary of named purposes that the base can serve when a spell is invented from it. A given spell base may serve multiple purposes (e.g., a CrAn base might be used to summon a riding mount or to conjure a guard creature), but a spell invented from that base is committed to exactly one use tag at invention time, reflecting the designer's intent. Some bases have only one meaningful use; others have several.

`SpellUseTag` replaces and subsumes the existing `TechniqueEffects` and `FormEffects` enums on `SpellBase`. Those enums currently function as a two-part lookup key for `GetSpellBaseForEffect(technique, form)` but carry no semantic meaning beyond indexing. Their existing combinations (`Detect+Aura`, `Detect+Vis`, `Detect+Gift`, etc.) become named `SpellUseTag` values, and the one-base-per-combination constraint is lifted: multiple spell bases across different TeFo combinations may now carry the same tag. A mage's art specialization determines which base they select when inventing a spell for a given purpose — `GetBestSpellBaseForTag(mage, tag)` evaluates all bases carrying the desired tag and proposes the one where the mage's lab total is highest. Helpers and activities that currently call `GetSpellBaseForEffect(technique, form)` switch to this call; `GetBestSpell(SpellBase)` gains a companion `GetBestSpell(SpellUseTag)` for inventory lookups.

**Form-constrained tag lookup.** Some use tags are satisfiable by spell bases across many Forms, and the right Form depends on context. `GetBestSpellBaseForTag` accepts an optional `formHint` parameter. When absent, selection is purely by lab total — the mage invents whatever Form they are strongest in. When present, the selection weights toward the hinted Form, reflecting a specific need. This is the primary interface between the Scenario Manager and the spell selection system: a Fear token generated by a specific threat carries the relevant Form as an attribute, and the resulting goal passes it as a hint. A mage threatened by a fire specialist needs `PersonalProtection[Ignem]`, not `PersonalProtection` in their strongest Form. If their Ignem lab total is weak, that is a genuine dilemma — invent a suboptimal ward, improve Ignem first, acquire a lab text, or avoid the conflict — and the system should surface it rather than silently substituting a convenient alternative.

Prestige- and reputation-related classification continues to use the existing `SpellTag` enum (Offensive, Defensive, Healing, Utility, etc.), which is orthogonal to `SpellUseTag` and is not replaced by this system.

**Defined Use Tags — initial set.** Each tag maps to a distinct benefit calculation in a specific helper. The granularity follows from that constraint: two spell purposes that feed into the same formula share a tag; purposes with different formulas or different goal contexts get different tags.

- *Detection:* `FindAura`, `FindVis`, `FindGift` — one tag per Find activity formula; direct mappings from the existing `Detect+Aura`, `Detect+Vis`, `Detect+Gift` effect pairs. Currently the only tags in active use.

- *Travel:* `TravelSpeed` — anything reducing transit time, feeding a single `TravelEfficiencyHelper` formula. Satisfiable by ReCo (teleportation, flight), MuCo (self-transformation into a fast-moving form), CrAn (summon a riding animal), MuAn (transform an existing animal into a mount), among others. Different magi arrive at the same tag via different TeFo paths determined by their art strengths.

- *Provisions:* `FoodProvision` — creates food or water, displacing mundane provisioning time. Satisfiable by CrAn (food animals or products), CrHe (plant-based food), CrAq (drinking water). Note that CrAn bases carry both `TravelSpeed` and `FoodProvision` tags, since a summoned animal serves either purpose depending on invention intent.

- *Construction:* `Construction` — building, shaping, and clearing for covenant establishment. Satisfiable by ReTe (move and shape stone), CrTe (create stone or metal), PeHe (clear land), ReHe (shape timber). Benefit is uniformly labor-seasons displaced regardless of Form.

- *Personal protection:* `PersonalProtection` — Rego ward bases across all Forms (ReAn, ReAq, ReAu, ReCo, ReHe, ReIg, ReIm, ReMe, ReTe, ReVi). Background desire picks by lab total; reactive desire from the Scenario Manager supplies a Form hint. This tag spans all Forms deliberately: Bonisagus' path to the Parma Magica breakthrough is precisely the systematic invention of form-specific wards, and the `PersonalProtection` tag is what makes each of those inventions practically motivated rather than purely research-driven.

- *Mundane social:* `MundaneSocial` — spells reducing the social friction of the Gift in dealings with non-magi. Primarily ReMe effects. Depends on the mundane world interaction model (§2.9.2) for its benefit calculation; included here for completeness.

- *Existing bases with narrow uses:* `QuantifyVis`, `CreateAura`, `CreateVis`, `ManipulateVis`, `ControlAura` — direct mappings from existing defined bases with no change in behavior. The former `WardMagic` tag is absorbed into `PersonalProtection[Vim]`.

**Migration notes.** The `TechniqueEffects` and `FormEffects` properties on `SpellBase` will be removed once the `SpellUseTag` set is in place. Two non-lookup construction sites require separate attention: the fallback in `ResearchService` that hard-codes `Detect+Aura` when no base matches a desired magnitude should instead select a use tag from the art pair's available bases; the dynamic base construction in `PursueIdeaGoal` that hard-codes `Manipulate+Animal` for un-catalogued ideas should derive use tags from the art pair's standard associations.
...
The art pairs associated with a breakthrough definition determine which TeFo combinations the research service draws from when generating experimental spells. This is a meaningful design choice: the `ParmaMagicaBreakthrough` should be associated with Rego + all Forms, not only ReVi. Bonisagus' historical path to the generalized magical protection of Parma Magica runs through the systematic invention of form-specific wards — protecting against animals, fire, earth, weather, and so on — before achieving the universal principle. Associating the breakthrough with the full Rego+Forms set produces this narrative organically: each experimental phase picks a Form (by highest lab total or randomly from those not yet covered), and Bonisagus accumulates a portfolio of practical protection spells carrying the `PersonalProtection` tag as a natural byproduct of his research. The practical and research motivations reinforce each other rather than competing.
...
**Spell Invention Phase 2.** The current spell invention system is mechanically functional but motivationally thin. Two distinct tracks are planned to enrich it.

The first track — **practical instrumental motivation** — does not require the Scenario Manager. The principle is consistent with the existing helper architecture: the AI weighs the opportunity cost of inventing a spell against the seasonal benefit it produces, using the same vis-per-season conversion that governs all activity decisions. Spells in this track are not prerequisites that gate other activities; they are expediters that shift the opportunity-cost calculation. A mage could always do things the mundane way — hire laborers, travel slowly, forage for food — but magic compresses the time cost, and the AI should weigh one season of spell invention against the accumulated time savings it yields.

Two categories of practical spell fall naturally under this track in the Founding era:

*Travel efficiency.* Movement and travel spells (primarily ReCo and ReAu effects) reduce the time cost of journeys, directly benefiting any activity that requires reaching a remote location. Because travel efficiency is relevant to all Find-type activities — finding auras, vis sources, apprentices, and hedge magi — the benefit calculation belongs in a shared `TravelEfficiencyHelper` invoked across all Find helpers, rather than being wired separately into each one. The desire for a travel spell is proportional to the expected number of Find trips per year and the fraction of a season each trip would save.

*Covenant provisions and construction.* Spells in the physical Forms — CrAn and CrHe for food, CrAq for water, CrTe and ReTe for construction and shelter — reduce the mundane labor and time costs of establishing and maintaining a covenant. These spells have smaller and more one-time benefits than travel magic, so they will tend to be lower priority; but their desire flows from the same opportunity-cost calculation. A `CovenantResourceHelper` would evaluate the seasonal cost of handling these needs mundanely against the season cost of inventing the relevant spell.

Both categories require spell bases — with appropriate `SpellUseTag` values — to exist in the catalog before the helpers can evaluate them. The first wave of spell base expansion should cover the bases needed to satisfy the `Travel`, `Mount`, `FoodProvision`, and `Construction` tags across a range of TeFo combinations (see Spell Use Tags above). These bases have concrete, calculable practical value that the AI can reason about without external event injection. Rego effects on mundane social interactions (ReMe to reduce the friction of the Gift's social penalty in dealings with non-magi) are also a candidate for this track, though they depend on the mundane world interaction model (§2.9.2) being sufficiently developed.

The second track — **reactive and story-driven motivation** — does require the Scenario Manager (§2.9.1). A mage facing a demon incursion and lacking PeVi spells is motivated to acquire them; a covenant under mundane threat needs defensive and offensive magic. These motivations cannot be derived from standing character state and are deferred to the phase when the Scenario Manager is implemented.

The interface between the Scenario Manager and the spell selection system runs through the Form-constrained tag lookup described above. When the Scenario Manager injects a threat, the Appraisal Engine generates a Fear token carrying the nature of the threat — including the relevant Form where applicable. Goal generation from that Fear token produces a spell desire with both a `SpellUseTag` and a `formHint`. A fire mage threatening Wizard's War produces `PersonalProtection[Ignem]`; a demon incursion produces `PersonalProtection[Vim]` or a new offensive tag with the same constraint. This keeps the tag vocabulary stable while allowing the Scenario Manager to direct spell desire with precision.
...
- **Recruitment:** Magi can search for and recruit hedge wizards into the Order. The pipeline has two phases: finding (Folk Ken + Perception + Area Lore/Etiquette + InVi magic bonus, against an ease factor) and persuasion (contested roll, currently defaulting to success). On success, the hedge mage converts to a Hermetic magus, joins the recruiter's master's covenant as a Visitor, and their tradition's potential breakthroughs become available as research targets.
  ...
  The Founding Scenario uses the recruitment pipeline with goal-driven behavior rather than scripting. Trianoma is initialized with a permanent `RecruitHedgeMageGoal`; her high Fairness and Sociability facets produce strong desire for it. Bonisagus responds organically when an unfamiliar tradition appears at his covenant, generating tradition integration goals and Opening-the-Gift goals from his standing personality and goal machinery. The historical Founding emerges from the interaction of their goals and circumstances — no fixed action sequence is hard-coded. Other scenarios may seed initial conditions that make specific historical outcomes highly probable (see §2.9.1), but the outcomes themselves remain products of the simulation. The goal generation steps for tradition integration and for Opening the Gift are not yet implemented; see §2.9.6 for the milestone-by-milestone breakdown of what is required.
...
- **Exploration:** Characters can search for magical auras and sources of vis. All Find-type activities — aura finding, vis finding, apprentice finding, and hedge mage recruitment — share a common dependence on travel to remote locations. A planned `TravelEfficiencyHelper` will be invoked across all Find helpers to evaluate whether inventing a movement or travel spell is a better use of a season than another Find trip; see §2.3.1 Spell Invention Phase 2.
...
The Founding Scenario is the primary validation vehicle for the simulation's social and magical systems. Unlike later scenarios, it has a known historical arc: roughly thirty magi gathered by Trianoma and others over approximately thirty years before the formal founding of the Order in 767 AD. The scenario is not scripted — the historical outcome should emerge from the interaction of hand-authored characters and goal-driven behavior — but the known arc provides a meaningful benchmark: does the simulation produce something recognizable as the Founding?

The scenario is organized into six milestones, each testing a specific set of goal types and system interactions. Milestones are not hard gates; they describe the behavioral arc that should emerge once the required systems are in place.

**Milestone 1: Bonisagus in isolation.** Bonisagus begins the simulation alone with a laboratory and basic Hermetic arts. His behavior should be self-directed and coherent: pursuing research goals driven by Pride and Inquisitiveness, managing longevity, improving his Lab Total over time, and generating spell invention goals motivated by his research agenda. This milestone validates the core cognitive stack — `AvoidDecrepitudeGoal`, `PursueIdeaGoal`, `AbilityScoreGoal` — and the tradition integration pipeline for his own native concepts.

*Requires:* Core goal machinery (implemented). Spell invention motivation is currently thin and will deepen once the Scenario Manager is generating problems (§2.9.1).

**Milestone 2: Trianoma arrives.** Trianoma joins as a Visitor at Bonisagus's covenant. This milestone exercises two new goal types that are not yet implemented:

- *Milestone 2a — Tradition integration goal:* Bonisagus generates a goal to research Trianoma's tradition concepts. The generative condition is contact with an unfamiliar tradition carrying potential concepts; Bonisagus's Inquisitiveness lowers the threshold. The goal generation step for tradition integration is currently a known gap (§2.3.1) and must be wired before this milestone can proceed.

- *Milestone 2b — Open Gift and teach Parma:* Bonisagus generates a goal to re-open Trianoma's Gift under his tradition and to teach her Parma Magica. This requires a new `OpenGiftGoal` (or a composite that wraps both the Opening lab activity and the collaborative Parma teaching activity) that the GoalGenerator can emit. The primary generative conditions are Bonisagus's Fairness facet and his belief about Trianoma's potential as a fellow maga. The two sub-goals may be modeled as sequential conditions within a single goal or as separate intentions competing for priority.

Milestones 2a and 2b may proceed in parallel or sequence depending on how Bonisagus's goals compete for seasonal priority.

*Requires:* Tradition integration goal generation (not implemented). `OpenGiftGoal` (not implemented).

**Milestone 3: Trianoma searches.** After her Gift is re-opened and she has learned Parma, Trianoma begins actively seeking other hedge magi to bring into the Order. Her permanent `RecruitHedgeMageGoal` drives this behavior. `FindApprenticeGoal` is structurally similar in its use of `FindApprenticeHelper`; both must be wired into `TryGenerateSocialIntentions()`. For Trianoma specifically, the `RecruitHedgeMageGoal` is hand-authored as a permanent standing intention rather than generated dynamically; the dynamic generation path serves other characters who acquire recruiting motivation through their personality and circumstances.

*Requires:* `FindApprenticeGoal` wired into `TryGenerateSocialIntentions()` (not yet connected). `RecruitHedgeMageGoal` already characterized for Trianoma. `TravelEfficiencyHelper` (not yet implemented) would make Trianoma's recruiting activity the primary driver of travel spell desire — her high trip frequency gives travel magic its largest return.

**Milestone 4: A hedge magus arrives (Mercere).** Trianoma brings a recruited hedge mage back to the covenant. Bonisagus repeats the tradition integration and Opening process from Milestone 2 for the new arrival, while Trianoma resumes searching. The system should handle multiple cycles of this without special-casing: Bonisagus's Fairness and his standing integration goals should fire organically for any unfamiliar tradition that appears at his covenant.

*Requires:* Milestone 2 complete. Repeated application of the same goal types.

**Milestone 5: The new magus founds a covenant.** Once Mercere has been re-opened and has learned Parma, he should develop the motivation to establish his own covenant. `FoundCovenantGoal` is implemented — it chains to `IsCovenantFounderCondition` → `HasAuraCondition` → `FindNewAuraHelper` and `FindVisSourceHelper`, all of which are functional — but the goal is not generated by the `GoalGenerator`. It must be wired into `TryGenerateSocialIntentions()` with the following generative conditions:

- The character has been a Visitor (not a full member or founder) for a sufficient number of seasons.
- The character has a functioning laboratory and enough magical development to operate independently.
- Personality: sufficient autonomy drive. The specific personality mapping requires design; low Agreeableness and high Openness to Experience are plausible candidates.
- Belief: the character believes they have identified, or can plausibly find, a suitable aura.

*Requires:* `FoundCovenantGoal` wired into `TryGenerateSocialIntentions()` with generative conditions designed and implemented. A `CovenantResourceHelper` (see §2.3.1 Spell Invention Phase 2) would give the newly independent mage instrumental motivation to invent CrAn, CrHe, CrAq, and ReTe/CrTe spells to expedite provisioning and construction — the same opportunity-cost model as the existing lab-total helpers.

**Milestone 6: Trianoma as ambassador.** As more magi are integrated and new covenants are founded, Trianoma's role evolves: she divides her time between recruiting new hedge magi and supporting newly founded covenants. The mentoring behavior is a new goal type not yet designed — a `MentorCovenantGoal` or similar — that fires when Trianoma's Fairness generates helping goals toward a specific covenant's needs. The split between recruiting and mentoring should emerge from competing goal priorities rather than scripted alternation.

*Requires:* Milestone 5 complete. `MentorCovenantGoal` not yet designed.
