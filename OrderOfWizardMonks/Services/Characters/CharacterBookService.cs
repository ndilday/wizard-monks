...
namespace WizardMonks.Services.Characters
{
    public static class CharacterBookService
    {
        ...
        public static ABook WriteBook(this Character author, Ability topic, string name, Ability exposureAbility, double desiredLevel = 0)
        {
            // confirm writing pre-reqs met
            if(!CanWrite(author))
            {
                return null;
            }
            // grant exposure experience
            author.GetAbility(exposureAbility).AddExperience(2);
            ABook book;
            // TODO: When should books moved from the owned list to the covenant library?
            if (desiredLevel == 1000)
            {
                book = WriteTractatus(author, topic, name);
            }
            else
            {
                book = WriteSumma(author, topic, name, desiredLevel);
            }
            if(book != null)
            {
                author.AddBookToCollection(book);
                author.BooksWritten.Add(book);
            }
            return book;
        }

        private static Tractatus WriteTractatus(Character author, Ability topic, string name)
        {
            Tractatus t = new()
            {
                Author = author,
                Quality = author.GetAttribute(AttributeType.Communication).Value + 6,
                Topic = topic,
                Title = name
            };
            ...
            if (Die.Instance.RollDouble() < 0.10) // Configurable constant here
            {
                ...
                t.BeliefPayload.Add(
                    new Belief(
                        randomFacet.ToString(), 
                        BeliefToReputationNormalizer.FromPersonalityFacet(author.Personality.GetFacet(randomFacet))));
            }
            return t;
        }
        ...
        private static Summa WriteSumma(Character author, Ability topic, string name, double desiredLevel)
        {
            Summa s;
            CharacterAbilityBase ability = author.GetAbility(topic);
            Summa previousWork = author.IncompleteBooks.Where(b => b.Title == name).FirstOrDefault();
            if (previousWork == null)
            {
                ...
                s = new Summa()
                {
                    ...
                    Quality = MagicArts.IsArt(ability.Ability) ?
                        author.GetAttribute(AttributeType.Communication).Value + difference + 6 :
                        author.GetAttribute(AttributeType.Communication).Value + difference * 3 + 6
                };
            }
            else
            {
            ...
            s.PointsComplete += author.GetAttribute(AttributeType.Communication).Value + author.GetAbility(author.WritingLanguage).Value;
            if (s.PointsComplete >= s.GetWritingPointsNeeded())
            {
                author.AddBookToCollection(s);
                if (previousWork != null)
                {
                    author.IncompleteBooks.Remove(previousWork);
                }
                return s;
            }
            return null;
